...
MICtArray.forEach(function(miDef){
	if(miDef.length==0) {
		var mi = doc.createElementNS(XUL_NS,'menuseparator');
	} else {
		var mi = doc.createElementNS(XUL_NS,'menuitem');
...
function doIContextDummy(e) {						console.log("doIContextDummy");console.log(e.target.parentNode.triggerNode);
...
function bmEdit(bmItem) {						console.log("bmEdit");//,bmItem
	...
	/*PlacesUIUtils.showBookmarkDialog({ 
		...
		type:	"bookmark",
		itemId: bmItem.bmId,
		hiddenRows: ["folderPicker"]				//Don't know why, but otherwise empty since FF 40
	}, window);*/
}
